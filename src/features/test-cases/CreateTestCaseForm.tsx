...
export function CreateTestCaseForm({ architectureId, onSuccess }: CreateTestCaseFormProps) {
  const [successMessage, setSuccessMessage] = useState('');

  const { create, isLoading, errorMessage: apiErrorMessage } =
    useCreateTestCase(architectureId);
  ...
  const onSubmit = async (data: CreateTestCaseInput) => {
    ...
    try {
      await create(data);
      setSuccessMessage('Caso de prueba creado correctamente');
      reset();
      onSuccess?.();
  ...
  return (
    <div className="w-full max-w-2xl mx-auto p-6">
      <h2 className="text-xl font-semibold mb-6">Nuevo Caso de Prueba</h2>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        {/* Submit */}
        <Button type="submit" disabled={isLoading || !architectureId} className="w-full">
          {isLoading ? (
            <>
